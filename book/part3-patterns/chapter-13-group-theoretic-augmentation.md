# Chapter {{ch:group-theoretic-augmentation}}: Group-Theoretic Data Augmentation
...
## {{ch:group-theoretic-augmentation}}.1 Symmetry as a Computational Resource

### {{ch:group-theoretic-augmentation}}.1.1 The Cost of Ignorance
...
### {{ch:group-theoretic-augmentation}}.1.2 Three Ways to Exploit Symmetry
...
This chapter focuses primarily on data augmentation, which is the most widely applicable and the strategy implemented in the ARC-AGI codebase. Section {{ch:group-theoretic-augmentation}}.5 discusses equivariant architectures as a complement.

---

## {{ch:group-theoretic-augmentation}}.2 The Dihedral Group $D_4$

### {{ch:group-theoretic-augmentation}}.2.1 Definition and Elements
...
### {{ch:group-theoretic-augmentation}}.2.2 The Group Multiplication Table
...
### {{ch:group-theoretic-augmentation}}.2.3 Group Actions on Grids
...
## {{ch:group-theoretic-augmentation}}.3 Implementation: D4 Augmentation for ARC-AGI

### {{ch:group-theoretic-augmentation}}.3.1 The Core Transform Functions
...
### {{ch:group-theoretic-augmentation}}.3.2 Generating the Full Orbit
...
### {{ch:group-theoretic-augmentation}}.3.3 Consistent Augmentation of Input-Output Pairs
...
## {{ch:group-theoretic-augmentation}}.4 The ARC-AGI Application

### {{ch:group-theoretic-augmentation}}.4.1 Why ARC Puzzles Have Natural $D_4$ Symmetry
...
This connects directly to the hyperbolic rule encoding developed in Chapter {{ch:hyperbolic-geometry}}. The `HyperbolicRuleEncoder` maps transformation rules into the Poincare ball, where abstract rules cluster near the origin and specific sub-rules occupy the periphery. When the training pairs are augmented with $D_4$ transforms, the pair encoder sees eight orientations of the same abstract rule. The resulting hyperbolic embeddings cluster more tightly --- the augmented pairs reinforce the abstract rule signal while averaging out the orientation-specific noise.

### {{ch:group-theoretic-augmentation}}.4.2 Augmentation in the Solver Pipeline
...
### {{ch:group-theoretic-augmentation}}.4.3 Interaction with Structure Probing

Chapter {{ch:adversarial-probing}} introduced the structure probing framework, which applies parametric transforms to grids and measures latent displacement to discover what invariances the model has learned. The connection to $D_4$ augmentation is direct: the `rotate`, `reflect_h`, and `reflect_v` parametric transforms in the probe suite are exactly the generators of $D_4$.
...
## {{ch:group-theoretic-augmentation}}.5 Equivariant Architectures

### {{ch:group-theoretic-augmentation}}.5.1 From Augmentation to Equivariance
...
### {{ch:group-theoretic-augmentation}}.5.2 Group Convolutions
...
### {{ch:group-theoretic-augmentation}}.5.3 Why ARC Uses Augmentation Instead
...
Third, the solver combines $D_4$ transforms with color permutations. Building an architecture equivariant to $D_4 \times S_9$ is substantially more complex than building one equivariant to $D_4$ alone, while augmenting with elements of the product group is straightforward (as shown in Section {{ch:group-theoretic-augmentation}}.3.3).

---

## {{ch:group-theoretic-augmentation}}.6 The Orbit-Stabilizer Theorem in Practice

### {{ch:group-theoretic-augmentation}}.6.1 Detecting Redundant Augmentations
...
### {{ch:group-theoretic-augmentation}}.6.2 Burnside's Lemma and Counting Distinct Patterns
...
## {{ch:group-theoretic-augmentation}}.7 Extending Beyond $D_4$

### {{ch:group-theoretic-augmentation}}.7.1 Color Permutation Groups
...
### {{ch:group-theoretic-augmentation}}.7.2 Translation Groups
...
### {{ch:group-theoretic-augmentation}}.7.3 Scale Symmetries
...
### {{ch:group-theoretic-augmentation}}.7.4 Continuous Symmetries
...
## {{ch:group-theoretic-augmentation}}.8 The Algebra of Augmentation Pipelines

### {{ch:group-theoretic-augmentation}}.8.1 Augmentation as a Group Homomorphism
...
This test is not merely pedagogical. Augmentation bugs --- where the implemented transforms do not form a group --- are a real source of training degradation. A common mistake is implementing reflections as views rather than copies (Section {{ch:group-theoretic-augmentation}}.3.1), which causes subsequent rotations to modify the original, breaking the group structure.

---

## {{ch:group-theoretic-augmentation}}.9 Connection to the Geometric Framework

### {{ch:group-theoretic-augmentation}}.9.1 Symmetry in the State Space

The geometric framework of this book represents model configurations as points in a multi-dimensional state space (Chapter {{ch:why-geometry}}). Symmetry groups act on this state space, mapping one configuration to an equivalent one. The orbit of a configuration under the symmetry group defines an *equivalence class*, and the quotient space $\mathcal{S} / G$ (the space of equivalence classes) is the effective search space.
...
### {{ch:group-theoretic-augmentation}}.9.2 Orbits as Geodesics
...
## {{ch:group-theoretic-augmentation}}.10 Practical Guidelines

### {{ch:group-theoretic-augmentation}}.10.1 When to Augment
...
### {{ch:group-theoretic-augmentation}}.10.2 How Many Augmentations
...
### {{ch:group-theoretic-augmentation}}.10.3 Verifying Augmentation Effectiveness
...
2. **Probing invariance.** Use the structure probe (Chapter {{ch:adversarial-probing}}) to measure latent displacement under group transforms. Post-augmentation displacement should be lower than pre-augmentation displacement for the targeted transforms.
...
## {{ch:group-theoretic-augmentation}}.11 Summary
...
4. **Augmentation complements probing.** The structure probe (Chapter {{ch:adversarial-probing}}) measures whether augmentation has been absorbed; the hyperbolic rule encoder (Chapter {{ch:hyperbolic-geometry}}) measures whether augmented training pairs produce consistent rule embeddings.
...
## {{ch:group-theoretic-augmentation}}.12 Forward: Symmetry-Aware Fuzzing

Chapter {{ch:gradient-reversal}} takes the symmetry framework from augmentation to *fuzzing*. Where this chapter used symmetry to generate *equivalent* training examples, Chapter {{ch:gradient-reversal}} uses symmetry to generate *adversarial* test cases --- inputs designed to expose failures in the model's learned invariances. The key insight is that a model's failure to respect a known symmetry is a *bug*, and fuzzing the symmetry group's orbit is a targeted strategy for finding such bugs. The probing framework of Chapter {{ch:adversarial-probing}} provides the detection mechanism; the group theory of this chapter provides the space of perturbations to search; and Chapter {{ch:gradient-reversal}} combines them into a systematic adversarial testing methodology.
