# Chapter {{ch:subset-enumeration}}: The Subset Enumeration Pattern
...
The pattern connects backward to the motivating example of Chapter {{ch:why-geometry}}, where subset enumeration first appeared as Step 1 of the geometric validation pipeline, and forward to Chapter {{ch:pareto-optimization}}, where Pareto frontier analysis operates over the space of `SubsetResult` objects to identify non-dominated tradeoffs between model complexity and predictive quality.

---

## {{ch:subset-enumeration}}.1 The Combinatorial Explosion

### {{ch:subset-enumeration}}.1.1 Counting Subsets
...
For the defect prediction example introduced in Chapter {{ch:why-geometry}}, $n = 5$ feature groups (Size, Complexity, Halstead, OO, Process). The subset counts are:
...
### {{ch:subset-enumeration}}.1.2 The Scaling Boundary
...
### {{ch:subset-enumeration}}.1.3 Why Brute Force Beats Heuristics
...
## {{ch:subset-enumeration}}.2 The `optimize_subset` Algorithm
...
### {{ch:subset-enumeration}}.2.1 Log-Space Parameterization
...
### {{ch:subset-enumeration}}.2.2 Sentinel Values for Inactive Dimensions
...
### {{ch:subset-enumeration}}.2.3 Adaptive Search Strategy
...
## {{ch:subset-enumeration}}.3 `SubsetResult` as a Typed Data Structure
...
- **`errors`**: a dictionary of per-metric errors. For the defect prediction example, this might contain `{"Accuracy": 3.2, "Precision": -1.5, "Recall": 5.1, "F1": 2.8, "AUC": -0.3}`. The dictionary preserves the full multi-dimensional evaluation---exactly the information that, as Chapter {{ch:why-geometry}} argued, scalar summaries destroy.

- **`pareto_optimal`**: a boolean flag, initially `False`, set to `True` by the Pareto frontier analysis (Chapter {{ch:pareto-optimization}}) for configurations that are non-dominated in the (n_dims, MAE) plane. This flag allows downstream code to filter for Pareto-optimal results without re-computing the frontier.
...
This design follows the principle stated in Section {{ch:why-geometry}}.2.2: state vectors are immutable, dimensions are named not numbered, and the full multi-dimensional evaluation is preserved.

---

## {{ch:subset-enumeration}}.4 The `enumerate_subsets` Function
...
### {{ch:subset-enumeration}}.4.1 The Role of `max_dims`
...
### {{ch:subset-enumeration}}.4.2 Integration with the Pipeline
...
## {{ch:subset-enumeration}}.5 Practical Heuristics

### {{ch:subset-enumeration}}.5.1 Choosing `max_dims`
...
### {{ch:subset-enumeration}}.5.2 Early Stopping
...
### {{ch:subset-enumeration}}.5.3 Tuning `n_grid` and `n_random`
...
### {{ch:subset-enumeration}}.5.4 The Zero-Dimensional Baseline
...
### {{ch:subset-enumeration}}.6.2 The Evaluation Function
...
### {{ch:subset-enumeration}}.6.3 What the Enumeration Reveals
...
### {{ch:subset-enumeration}}.6.4 Connecting to the Pareto Frontier

The 30 `SubsetResult` objects, when plotted in the $(k, \text{MAE})$ plane, define a point cloud from which the Pareto frontier is extracted (Chapter {{ch:pareto-optimization}}). The frontier typically contains four points:
...
### {{ch:subset-enumeration}}.6.5 The Ground Truth Test
...
## {{ch:subset-enumeration}}.7 Computational Considerations

### {{ch:subset-enumeration}}.7.1 Parallelism
...
### {{ch:subset-enumeration}}.7.2 Memory
...
### {{ch:subset-enumeration}}.7.3 Reproducibility
...
## {{ch:subset-enumeration}}.8 Relationship to Other Patterns

### {{ch:subset-enumeration}}.8.1 Subset Enumeration vs. Forward/Backward Selection
...
### {{ch:subset-enumeration}}.8.2 Subset Enumeration and Sensitivity Analysis
