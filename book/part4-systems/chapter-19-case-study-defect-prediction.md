# Chapter {{ch:case-study-defect-prediction}}: Case Study --- Software Defect Prediction

> *"All models are wrong, but some are useful."*
> --- George E. P. Box (1976)

Chapter {{ch:why-geometry}} opened this book with a motivating example: a defect prediction model evaluated at 84% accuracy, and the long list of questions that scalar metric could not answer. The intervening chapters developed the geometric tools---subset enumeration, Pareto frontier analysis, sensitivity profiling, the Model Robustness Index, adversarial threshold search---needed to ask and answer those questions rigorously. This chapter brings the full toolchain to bear on a single, complete worked example. We start with a dataset, build an evaluation function, run the entire structural fuzzing campaign, and interpret every output. The goal is not merely to demonstrate the API but to show how geometric reasoning transforms the practice of model validation: from a single number to a multi-dimensional portrait of model behavior, fragility, and opportunity.
...
## {{ch:case-study-defect-prediction}}.1 The Dataset and Feature Groups

### {{ch:case-study-defect-prediction}}.1.1 Feature Architecture
...
The grouping itself is a modeling decision. One could split Halstead into "volume" and "effort" sub-groups, or merge Size and Halstead into a single "scale" dimension. Chapter {{ch:hyperbolic-geometry}} discusses the principles for constructing dimension groupings. Here, we follow the standard five-group decomposition because it aligns with the domain taxonomy and produces a space small enough for exhaustive analysis.

### {{ch:case-study-defect-prediction}}.1.2 Ground Truth Structure
...
### {{ch:case-study-defect-prediction}}.1.3 Generating the Data
...
### {{ch:case-study-defect-prediction}}.2.2 Implementation
...
### {{ch:case-study-defect-prediction}}.2.3 A Quick Sanity Check
...
## {{ch:case-study-defect-prediction}}.3 Running the Full Campaign
...
### {{ch:case-study-defect-prediction}}.3.1 Campaign Configuration
...
### {{ch:case-study-defect-prediction}}.3.2 The Six-Stage Pipeline
...
## {{ch:case-study-defect-prediction}}.4 Interpreting Subset Enumeration Results

### {{ch:case-study-defect-prediction}}.4.1 The Full Enumeration Table
...
### {{ch:case-study-defect-prediction}}.4.2 Reading the Enumeration
...
### {{ch:case-study-defect-prediction}}.4.3 The Per-Target Error Vectors
...
A configuration might achieve low MAE overall but fall short on Recall while exceeding the Accuracy target. The error vector makes these tradeoffs explicit. This is precisely the information that the Scalar Irrecoverability Theorem (Chapter {{ch:why-geometry}}, Section {{ch:why-geometry}}.1.1) proves cannot be recovered from a single aggregate metric.

---

## {{ch:case-study-defect-prediction}}.5 Analyzing the Pareto Frontier

### {{ch:case-study-defect-prediction}}.5.1 Extracting the Frontier
...
### {{ch:case-study-defect-prediction}}.5.2 Reading the Frontier
...
## {{ch:case-study-defect-prediction}}.6 Sensitivity Profiling Results

### {{ch:case-study-defect-prediction}}.6.1 Ablation Rankings

Sensitivity profiling (Chapter {{ch:equilibrium-on-manifolds}}) ablates each dimension from the best baseline configuration and measures the resulting increase in MAE. The ranking reveals which dimensions the model depends on most:
...
### {{ch:case-study-defect-prediction}}.6.2 Interpreting the Profile
...
### {{ch:case-study-defect-prediction}}.6.3 Sensitivity vs. Subset Enumeration
...
## {{ch:case-study-defect-prediction}}.7 The Model Robustness Index

### {{ch:case-study-defect-prediction}}.7.1 MRI Computation

The Model Robustness Index (Chapter {{ch:equilibrium-on-manifolds}}) quantifies how sensitive the best configuration is to parameter perturbations. The framework perturbs the baseline parameters 300 times in log-space and measures the distribution of MAE deviations:
...
### {{ch:case-study-defect-prediction}}.7.2 Interpreting the MRI
...
### {{ch:case-study-defect-prediction}}.7.3 The Perturbation Distribution
...
## {{ch:case-study-defect-prediction}}.8 Adversarial Threshold Discovery

### {{ch:case-study-defect-prediction}}.8.1 Finding Tipping Points

The adversarial threshold search (Chapters {{ch:adversarial-robustness}}--10) goes beyond aggregate robustness to find *specific* parameter values where model behavior changes qualitatively. For each dimension, the framework searches in both directions (increase and decrease) from the baseline, looking for the first perturbation that changes any quality metric by more than the tolerance (0.5 in this campaign):
...
### {{ch:case-study-defect-prediction}}.8.2 Interpreting the Thresholds
...
### {{ch:case-study-defect-prediction}}.8.3 From Thresholds to Deployment Constraints
...
## {{ch:case-study-defect-prediction}}.9 Compositional Testing

### {{ch:case-study-defect-prediction}}.9.1 The Build Sequence

The compositional test (Chapter {{ch:spd-manifolds}}) constructs a greedy dimension-addition sequence, starting from a single dimension and iteratively adding the candidate that produces the largest MAE reduction:
...
### {{ch:case-study-defect-prediction}}.9.2 Interpreting the Sequence
...
### {{ch:case-study-defect-prediction}}.9.3 Comparing with Baselines
...
## {{ch:case-study-defect-prediction}}.10 Complete Walkthrough: From Data to Actionable Insights

### {{ch:case-study-defect-prediction}}.10.1 The Full Script
...
### {{ch:case-study-defect-prediction}}.10.2 Generating LaTeX Output
...
### {{ch:case-study-defect-prediction}}.10.3 The Actionable Insights
...
## {{ch:case-study-defect-prediction}}.11 Lessons Learned
...
### {{ch:case-study-defect-prediction}}.11.1 Geometric Analysis Recovers Ground Truth
...
### {{ch:case-study-defect-prediction}}.11.2 Negative Results Are Results
...
### {{ch:case-study-defect-prediction}}.11.3 Tail Risk Matters
...
### {{ch:case-study-defect-prediction}}.11.4 Geometry Composes
...
### {{ch:case-study-defect-prediction}}.11.5 The Framework Scales

The five-dimensional case study in this chapter is deliberately small, chosen for pedagogical clarity. The framework's architecture scales to higher-dimensional spaces. For $n$ dimensions with `max_subset_dims = k$, the enumeration cost is $\sum_{j=1}^{k} \binom{n}{j}$, which grows polynomially for fixed $k$. With $k = 3$ and $n = 10$, this is $10 + 45 + 120 = 175$ subsets---still feasible. For $n = 20$ or beyond, the enumeration becomes expensive and the greedy approaches (forward selection, backward elimination, compositional testing) serve as practical approximations. Chapter {{ch:scaling}} discusses scaling strategies for high-dimensional analysis spaces in detail.

---

## {{ch:case-study-defect-prediction}}.12 Connection to What Follows
...
Chapter {{ch:case-study-bioacoustics}} extends the case study methodology to multi-objective scenarios where the targets themselves conflict---where improving Recall necessarily degrades Precision, and the Pareto frontier is not a convenience but a fundamental representation of the tradeoff space. The techniques from Chapter {{ch:pareto-optimization}} (Pareto optimization) combine with the sensitivity and adversarial methods from this chapter to produce a richer, more nuanced portrait of model behavior in the presence of genuinely competing objectives.
