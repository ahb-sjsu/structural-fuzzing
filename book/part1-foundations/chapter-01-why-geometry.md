# Chapter {{ch:why-geometry}}: Why Geometry?
...
## {{ch:why-geometry}}.1 The Limitations of Scalar Metrics

### {{ch:why-geometry}}.1.1 The Scalar Irrecoverability Theorem
...
### {{ch:why-geometry}}.1.2 A Concrete Example: The Ultimatum Game
...
### {{ch:why-geometry}}.1.3 The Practical Damage
...
3. **Misleading comparisons.** Model A outperforms Model B on a scalar benchmark. But Model B is superior on three of five dimensions and inferior only on the two that the scalar over-weights. A Pareto analysis (Chapter {{ch:pareto-optimization}}) reveals that neither dominates the other---the comparison is fundamentally multi-dimensional.
...
## {{ch:why-geometry}}.2 Multi-dimensional State Spaces as First-Class Objects
...
### {{ch:why-geometry}}.2.1 States as Points in $\mathbb{R}^n$
...
- **Subspaces** correspond to subsets of dimensions, enabling systematic exploration of which combinations of attributes matter (Chapter {{ch:subset-enumeration}}) and which are redundant.
...
## {{ch:why-geometry}}.3 When Euclidean Space Is Not Enough
...
### {{ch:why-geometry}}.3.1 Hierarchical Data and Hyperbolic Geometry
...
Chapter {{ch:topological-data-analysis}} introduces persistent homology for practitioners and applies it to structural fuzzing, using topological features of the perturbation response surface to detect fragility patterns that the Model Robustness Index (Chapter {{ch:adversarial-robustness}}) would miss.

---

## {{ch:why-geometry}}.4 Overview of the Geometric Toolchain
...
The tools are designed to compose. A typical analysis pipeline might: construct a state space (Chapter {{ch:mahalanobis-distance}}), enumerate subsets to identify important dimensions (Chapter {{ch:subset-enumeration}}), compute the Pareto frontier to find non-dominated configurations (Chapter {{ch:pareto-optimization}}), apply the MRI to quantify robustness of each Pareto-optimal point (Chapter {{ch:adversarial-robustness}}), and then run adversarial search to locate exact tipping points for the most promising configurations (Chapter {{ch:adversarial-probing}}). Each step uses geometry to extract information that the previous step's scalar summary would discard.
...
## {{ch:why-geometry}}.5 A Motivating Example
...
### {{ch:why-geometry}}.5.1 The Standard Approach
...
### {{ch:why-geometry}}.5.2 The Geometric Approach
...
**Step 3: Sensitivity profiling (Chapter {{ch:adversarial-robustness}}).** Ablation reveals that removing Complexity increases MAE by 1.9 (the most important dimension), removing Process increases it by 1.2, removing Size increases it by 0.6, while removing OO or Halstead increases it by less than 0.2 each. The scalar metric "84% accuracy" hid the fact that the model is overwhelmingly dependent on two of its five feature groups.

**Step 4: Model Robustness Index (Chapter {{ch:adversarial-robustness}}).** The MRI perturbs the baseline configuration 300 times, measuring the distribution of MAE deviations:
...
## {{ch:why-geometry}}.6 What Comes Next

The remainder of Part I (Chapters {{ch:why-geometry}}--5) builds the mathematical and software foundations: from Mahalanobis distance and hyperbolic geometry through SPD manifolds and topological data analysis.

Part II (Chapters {{ch:pathfinding-on-manifolds}}--10) develops the geometric algorithms themselves, from pathfinding on manifolds through adversarial probing. Each chapter introduces a mathematical tool, motivates it with a concrete problem, and provides a complete implementation.

Part III (Chapters {{ch:subset-enumeration}}--15) applies the toolchain to systems-level problems: subset enumeration, compositional testing, group-theoretic augmentation, and beyond.

Part IV (Chapters {{ch:geometric-pipelines}}--20) addresses integration concerns: geometric pipelines, scaling to high-dimensional spaces, production deployment, and complete case studies in defect prediction and bioacoustics.
