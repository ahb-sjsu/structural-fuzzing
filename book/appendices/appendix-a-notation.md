...
The default metric on $\mathbb{R}^n$. Treats all dimensions identically. Introduced in Chapter {{ch:why-geometry}} and contrasted with alternatives in Chapters {{ch:mahalanobis-distance}}--4.

### A.3.2 Mahalanobis Distance

$$d_M(\mathbf{a}, \mathbf{b}) = \sqrt{(\mathbf{a} - \mathbf{b})^\top \Sigma^{-1} (\mathbf{a} - \mathbf{b})}$$

Generalization of Euclidean distance that accounts for different scales and correlations among dimensions. When $\Sigma = I$, reduces to Euclidean distance. The precision matrix $\Sigma^{-1}$ stretches distances along directions of low variance and compresses them along directions of high variance. Chapter {{ch:mahalanobis-distance}}.

### A.3.3 Hyperbolic (Poincare Ball) Distance

$$d_c(x, y) = \frac{2}{\sqrt{c}} \, \text{arctanh}\!\Bigl(\sqrt{c}\,\bigl\|(-x) \oplus_c y\bigr\|\Bigr)$$

Geodesic distance on the Poincare ball $\mathbb{B}^d_c$. Diverges as points approach the boundary. Also expressed via arccosh (Chapter {{ch:why-geometry}}):

$$d_{\mathbb{B}}(x, y) = \text{arccosh}\left(1 + 2\frac{\|x - y\|^2}{(1 - \|x\|^2)(1 - \|y\|^2)}\right)$$

The two formulations are equivalent for all $c > 0$. Chapter {{ch:why-geometry}}, Chapter {{ch:hyperbolic-geometry}}.

### A.3.4 Log-Euclidean Distance

$$d_{LE}(S_1, S_2) = \|\log(S_1) - \log(S_2)\|_F$$

Distance on the SPD manifold, where $\log$ denotes the matrix logarithm. Respects the multiplicative structure of positive definite matrices: eigenvalue ratios contribute equally regardless of magnitude. Chapter {{ch:spd-manifolds}}.

### A.3.5 Affine-Invariant Distance

$$d_{AI}(S_1, S_2) = \|\log(S_1^{-1/2} S_2 S_1^{-1/2})\|_F$$

Alternative Riemannian metric on SPD(n), invariant under congruence transformations $S \mapsto ASA^\top$. Mentioned in Chapter {{ch:spd-manifolds}}, Section {{ch:spd-manifolds}}.7.

### A.3.6 Frobenius Distance

$$d_F(S_1, S_2) = \|S_1 - S_2\|_F$$

Euclidean distance on matrices treated as flat vectors. Does not respect SPD geometry. Used as a baseline in Chapter {{ch:spd-manifolds}}.

### A.3.7 Geodesic Distance from Origin

$$d_c(\mathbf{0}, h) = \frac{2}{\sqrt{c}}\,\text{arctanh}\!\bigl(\sqrt{c}\,\|h\|\bigr)$$

Special case of hyperbolic distance measuring depth/specificity of a point on the Poincare ball. Monotonically increasing in $\|h\|$. Chapter {{ch:hyperbolic-geometry}}.
...
The group operation on the Poincare ball $\mathbb{B}^d_c$, generalizing vector addition to hyperbolic space. Non-commutative but gyrocommutative. Identity element is $\mathbf{0}$; inverse of $x$ is $-x$. Reduces to standard addition as $c \to 0$. Chapter {{ch:hyperbolic-geometry}}.
...
Maps a tangent vector $v \in T_x\mathbb{B}^d_c$ to a point on the manifold by following the geodesic from $x$ in direction $v$ for unit time. Chapter {{ch:hyperbolic-geometry}}.
...
Inverse of the exponential map. Returns the tangent vector at $x$ pointing toward $y$ with magnitude equal to the geodesic distance. Chapter {{ch:hyperbolic-geometry}}.

### A.4.4 Matrix Logarithm

$$\log(S) = U \cdot \text{diag}(\log \lambda_1, \ldots, \log \lambda_n) \cdot U^\top$$

where $S = U\Lambda U^\top$ is the eigendecomposition of SPD matrix $S$ with eigenvalues $\lambda_i > 0$. Maps from SPD(n) to Sym(n) (the tangent space). Chapter {{ch:spd-manifolds}}.

### A.4.5 Matrix Exponential

$$\exp(X) = U \cdot \text{diag}(e^{\mu_1}, \ldots, e^{\mu_n}) \cdot U^\top$$

where $X = U \cdot \text{diag}(\mu_1, \ldots, \mu_n) \cdot U^\top$ is the eigendecomposition of symmetric matrix $X$. Maps from Sym(n) to SPD(n). The inverse of the matrix logarithm. Chapter {{ch:spd-manifolds}}.

### A.4.6 Conformal Factor

$$\lambda_x^c = \frac{2}{1 - c\,\|x\|^2}$$

The Riemannian conformal factor on the Poincare ball. Relates the Poincare metric tensor to the Euclidean metric: $g_x^{\mathbb{B}} = (\lambda_x^c)^2\, g^E$. Diverges as $\|x\| \to 1/\sqrt{c}$, reflecting the exponential magnification of distances near the boundary. Chapter {{ch:hyperbolic-geometry}}.

### A.4.7 Lorentz Factor

$$\gamma_c(x) = \frac{1}{1 - c\,\|x\|^2}$$

Used in the Einstein midpoint computation. Differs from the conformal factor $\lambda_x^c$ by a factor of 2 because it is derived in the Klein model rather than the Poincare model. Chapter {{ch:hyperbolic-geometry}}.
...
Projects points back inside the open ball after floating-point drift. The parameter $r_{\max} < 1$ (typically $0.95$) provides a safety margin. Chapter {{ch:hyperbolic-geometry}}.

### A.4.9 Scalar Projection

$$\phi : \mathbb{R}^n \to \mathbb{R}^1, \quad \phi(\mathbf{v}) = \mathbf{w}^\top \mathbf{v}$$

Any linear projection from a multi-dimensional evaluation to a single scalar. The null space $\ker(\phi)$ has dimension $n - 1$, destroying $n - 1$ directions of information. Chapter {{ch:why-geometry}}.
...
Weighted average of points on the Poincare ball, derived from the Klein model. Upweights points near the boundary via the Lorentz factor $\gamma_c$. Reduces to the Euclidean weighted mean as $c \to 0$. Chapter {{ch:hyperbolic-geometry}}.

### A.5.2 Frechet Mean on SPD(n)

$$\bar{S}_{LE} = \exp\!\left(\sum_{i=1}^{k} w_i \log(S_i)\right)$$

The Frechet mean under the log-Euclidean metric. Minimizes $\sum_i w_i\, d_{LE}(S_i, M)^2$ over $M \in \text{SPD}(n)$. Closed-form: take the weighted mean in log-space, then exponentiate. Chapter {{ch:spd-manifolds}}.

### A.5.3 Geodesic Interpolation on SPD(n)

$$\gamma(t) = \exp\!\bigl((1-t)\log(S_0) + t\log(S_1)\bigr), \quad t \in [0, 1]$$

Linear interpolation in log-space, yielding the geodesic (shortest path) on the SPD manifold under the log-Euclidean metric. Chapter {{ch:spd-manifolds}}.
...
The Model Robustness Index combines mean deviation, 75th percentile, and 95th percentile of perturbation response into a single robustness score. The composite explicitly accounts for tail risk, unlike standard deviation which treats all deviations symmetrically. Chapter {{ch:why-geometry}} (introduced), Chapter {{ch:equilibrium-on-manifolds}} (developed).

### A.7.2 Softmax Choice Rule

$$P(\text{reject}) = \frac{\exp(-d_{\text{reject}} / T)}{\exp(-d_{\text{accept}} / T) + \exp(-d_{\text{reject}} / T)}$$

Probabilistic choice model based on Mahalanobis distances, where $T$ is a temperature parameter controlling decision sharpness. Chapter {{ch:mahalanobis-distance}}.

### A.7.3 Stake-Dependent Temperature

$$T(\text{stake}) = \max\left(T_{\text{floor}},\; T_{\text{base}} + \frac{T_\alpha}{\sqrt{\text{stake}}}\right)$$

Temperature as a function of economic stakes. High stakes yield low temperature (sharper decisions); low stakes yield high temperature (noisier decisions). Chapter {{ch:mahalanobis-distance}}.
...
With $H_0$ and $H_1$, this yields a 16-dimensional feature vector. Chapter {{ch:topological-data-analysis}}.
...
This mechanism enables subset enumeration: each combination of active/inactive dimensions defines a different sparsity pattern on $\Sigma^{-1}$. Chapters {{ch:why-geometry}}, {{ch:mahalanobis-distance}}.
...
Typical bounds are $\alpha = 0.01$, $\beta = 100$. This provides uniform resolution across orders of magnitude: a change from $0.01$ to $0.1$ and a change from $10$ to $100$ each span one decade and receive equal representation. Chapter {{ch:mahalanobis-distance}}.

For Cholesky diagonals, log-space is enforced via $\ell_{ii} = e^{\theta_i}$, mapping the unconstrained real line to strictly positive values. Chapter {{ch:mahalanobis-distance}}.
...
Hyperbolic curvature is parameterized as $-c$ where $c > 0$. The Poincare ball $\mathbb{B}^d_c$ has constant sectional curvature $-c$. Setting $c = 1$ gives standard hyperbolic geometry with curvature $-1$. Typical working range is $c \in [0.1, 2.0]$. Curvature may be treated as a learnable parameter. Chapter {{ch:hyperbolic-geometry}}.

### A.8.7 Immutability

State vectors, once constructed, are not modified in place. All operations (perturbation, projection, interpolation) produce new vectors. This convention prevents aliasing bugs and ensures trajectory reproducibility. Chapter {{ch:why-geometry}}.
